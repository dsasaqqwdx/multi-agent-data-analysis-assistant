...
def get_dataset_summary(df: pd.DataFrame) -> str:
    ...
    for col in df.columns:
        ...
        lines.append(
            f"{col} | dtype={dtype} | unique={unique} | missing={missing}"
        )
...
def determine_features(df: pd.DataFrame, target: str):
    features = []
    ...
    for col in df.columns:
        ...
        if likely_id and unique_ratio > 0.7:
            continue
...
def get_models(problem_type: str, train_size: int):
...
def detect_requested_model(query: str):
    query_lower = query.lower()
...
def extract_prediction_values(
    ...
    for feature in features:
        feature_pattern = re.escape(feature.lower())

        patterns = [
            rf"{feature_pattern}\s*(?:is|=|:)?\s*([a-zA-Z0-9.\-_ ]+?)(?=,|and|with|$)",
            rf"([0-9.]+)\s+{feature_pattern}"
        ]
...
def run_ml_agent(
    ...
    try:
        ...
        return (
            f"Model trained successfully, but prediction "
            f"failed: {e}"
        )
    ...
    if len(extracted_values) > 0:
        ...
        result_lines.extend(
            [
                "",
                "No feature values were clearly detected "
                "from your question.",
                "Missing feature values were handled "
...
if __name__ == "__main__":
    answer = run_ml_agent(
        query="Predict salary for years_experience 5",
